...
class AdFilters
{
    ...
    {
        $this->builder = $builder->whereNotNull('published_at')->whereHas('files');
        ...
        if ($this->request->filled('paginate') && $this->request->paginate == '1') {
            $limit = $this->request->input('limit', 15); // Default to 15 if not provided
            $page = $this->request->input('page', 0);    // Default to 0 if not provided
            $offset = $page * $limit;
    ...
    {
        // Always order by 'is_golden' first (descending, so 1 comes before 0)
        $this->builder->orderBy('is_golden', 'desc');

        // Apply additional ordering based on request if provided
        if ($this->request->has('order_by')) {
            ...
            switch ($orderField) {
                case 'price':
                    $this->builder->orderBy('price', $orderDirection);
                    break;
                ...
                case 'area':
                    $this->builder->orderBy('area', $orderDirection);
                    break;
            ...
            // Default secondary ordering if no order_by is specified
    ...
    protected function filterByCategoryId($categoryId)
    {
        $this->builder->where(function ($query) use ($categoryId) {
            if ($categoryId == 'rent') {
                ...
                $query->where('per_day_or_month', 1);
            } elseif ($categoryId == 0 || $categoryId == -1 || $categoryId == '-1' || $categoryId == 'sell' || $categoryId == 'rent') {
                // Do nothing
            } else {
                if ($categoryId != '') {
    ...
    protected function filterByCityMapLatitude()
    {
        ...
        if (
            $this->request->filled('city_map_latitude') &&
    ...
    protected function filterByPriceFrom()
    {
        if ($this->request->filled('price_from')) {
            $this->builder->where('price', '>=', $this->request->price_from);
        }
    }
    ...
    protected function filterByPriceTo()
    {
        if ($this->request->filled('price_to')) {
            $this->builder->where('price', '<=', $this->request->price_to);
        }
    }
        ...
        if ($this->request->filled('number_of_rooms')) {
            $this->builder->whereHas('adable', function ($query) {
                if (Schema::hasColumn($query->getModel()->getTable(), 'number_of_rooms')) {
                    $input = $this->request->number_of_rooms;

                    // Check if input starts with '+' for greater-than condition
                    if (strpos($input, '+') === 0) {
                        $number = ltrim($input, '+'); // Remove '+' and get the number
                        if (is_numeric($number)) {
                            $query->where('number_of_rooms', '>=', $number);
                        }
                    } else {
                        // Exact match for the number
                        if (is_numeric($input)) {
                            $query->where('number_of_rooms', $input);
                        }
                    }
                }
            });
    ...
    protected function filterByNumberOfFloors()
    {
        if ($this->request->has('number_of_floors') && $this->request->input('number_of_floors') == 'all') {
            return;
        }
        if ($this->request->filled('number_of_floors')) {
            $this->builder->whereHas('adable', function ($query) {
                if (Schema::hasColumn($query->getModel()->getTable(), 'number_of_floors')) {
                    $input = $this->request->number_of_floors;

                    // Check if input starts with '+' for greater-than condition
                    if (strpos($input, '+') === 0) {
                        $number = ltrim($input, '+'); // Remove '+' and get the number
                        if (is_numeric($number)) {
                            $query->where('number_of_floors', '>=', $number);
                        }
                    } else {
                        // Exact match for the number
                        if (is_numeric($input)) {
                            $query->where('number_of_floors', $input);
                        }
                    }
                }
            });
        ...
        if ($this->request->filled('number_of_halls')) {
            $this->builder->whereHas('adable', function ($query) {
                if (Schema::hasColumn($query->getModel()->getTable(), 'number_of_halls')) {
                    $input = $this->request->number_of_halls;

                    // Check if input starts with '+' for greater-than condition
                    if (strpos($input, '+') === 0) {
                        $number = ltrim($input, '+'); // Remove '+' and get the number
                        if (is_numeric($number)) {
                            $query->where('number_of_halls', '>=', $number);
                        }
                    } else {
                        // Exact match for the number
                        if (is_numeric($input)) {
                            $query->where('number_of_halls', $input);
                        }
                    }
                }
            });
        ...
        if ($this->request->filled('number_of_bathrooms')) {
            $this->builder->whereHas('adable', function ($query) {
                if (Schema::hasColumn($query->getModel()->getTable(), 'number_of_bathrooms')) {
                    $input = $this->request->number_of_bathrooms;

                    // Check if input starts with '+' for greater-than condition
                    if (strpos($input, '+') === 0) {
                        $number = ltrim($input, '+'); // Remove '+' and get the number
                        if (is_numeric($number)) {
                            $query->where('number_of_bathrooms', '>=', $number);
                        }
                    } else {
                        // Exact match for the number
                        if (is_numeric($input)) {
                            $query->where('number_of_bathrooms', $input);
                        }
                    }
                }
            });
    ...
    protected function filterByNumberOfUnitsFrom()
    {
        if ($this->request->filled('number_of_units_from')) {
            $this->builder->whereggaHas('adable', function ($query) {
    ...
    protected function filterByPerDayOrMonth($value)
    {
        if ($value == 1) {
            $this->builder->where('per_day_or_month', 1)
                ->where('for_rent', 1);
        } else {
            $this->builder->where(function ($query) {
                $query->where('per_day_or_month', 0)->orWhere('per_day_or_month', null);
            });
        }
    }
